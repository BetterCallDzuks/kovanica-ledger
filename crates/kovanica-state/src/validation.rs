...
//! assert!(dag.insert(Block::new(vec![genesis_id], 1, encode_block_payload(&[cb]))).is_ok());
//!
//! // A block whose payload is not valid transaction encoding is rejected at insert.
//! assert!(dag.insert(Block::new(vec![genesis_id], 1, b"not-transactions".to_vec())).is_err());
//! ```
//!
