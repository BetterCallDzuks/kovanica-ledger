...
//! through [`Dag::insert`], so all consensus data — past sets, colouring, blue
//! scores, tips — is recomputed deterministically and the rebuilt DAG is
...
//! dag.insert(Block::new(vec![a], 1, b"b".to_vec())).unwrap();
//!
...
pub fn decode_snapshot(bytes: &[u8]) -> Result<DagSnapshot, SnapshotError> {
    ...
    // Each block is at least 8 (parents len) + 16 (work) + 8 (payload len) = 32.
...
/// Encode a block's reconstruction data: parents, work, payload (length-prefixed,
/// little-endian). Not the id encoding — this is what rebuilds the `Block`.
...
impl<'a> Reader<'a> {
    ...
    fn read_block(&mut self) -> Result<Block, SnapshotError> {
        ...
        Ok(Block::new(parents, work, payload))
    }
}
...
mod tests {
    ...
    fn build() -> Dag {
        ...
        let _m = dag
            .insert(Block::new(vec![a, b], 3, b"m".to_vec()))
            .unwrap();
        dag
    }
    ...
    fn genesis_only_roundtrips() {
        let dag = Dag::new(1, Block::genesis(5, b"only".to_vec()));
        let restored = Dag::read_snapshot(&dag.write_snapshot()).unwrap();
        assert_eq!(restored.linearize(), dag.linearize());
